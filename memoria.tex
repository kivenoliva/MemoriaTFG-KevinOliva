...
Imaginemos que el desarrollador ha escrito esta línea de código
...
Si estamos leyendo código y pasamos por esta línea, se entiende perfectamente, sin más contexto, sabemos que estamos obteniendo los subtítulos, los cuales obtenemos de una función llamada ``extract-subtitles'', sabemos esto porque el nombre de la función se ha elegido correctamente y es informativo. También sabemos que para ello le pasamos como parámetros el id del vídeo y el id de los subtítulos.

Si estamos analizando código y tenemos todo el contexto de nuestro programa, es posible que no haga falta ir a la función ``extract-subtitles'', porque sabemos todo lo que nos hace falta con esta línea y podremos seguir leyendo el código que estamos analizando sin perder más tiempo.

Supongamos que por motivos de indentación, esta línea supera los 80 caracteres. El desarrollador ha intentado refactorizar su código pero no consigue quitar las indentaciones y como le ha saltado que no cumple la regla de estilo únicamente por la longitud de la línea, decide para no perder más tiempo, acortar el nombre de las variables, y forzando un caso extremo, también el de la función, de tal forma que la línea quedaría con menos de 80 caracteres y así cumplir con la regla. De forma que la línea se queda de la siguiente forma:

\begin{verbatim}
'subtitles': self.e_s(v_id, v_s_id, hl) 
\end{verbatim}

En este caso, la línea en sí carece de sentido alguno, si estamos analizando código seríamos incapaces de saber como estamos obteniendo la variable ``subtitles'', necesitaríamos irnos a la función para intentar entender que hace, y que parámetros le están llegando. La línea ahora cumple las reglas de estilo, pero es ilegible.
...
Para esto nacieron las guías de estilo de código. El principal objetivo de una guía de estilo es que nuestro código sea más fácil de leer, compartir y analizar. Facilita la consistencia entre programas de distintos usuarios y hace que nuestro código sea mucho más mantenible.

Las guías de estilo es algo que está generalizado en los distintos lenguajes de programación, por ejemplo para PHP tenemos  PSR-2\footnote{http://www.php-fig.org/psr/psr-2/}, en Javascript una de las más notables es Crockford\footnote{http://javascript.crockford.com/code.html}. No hay una sola guía de estilo para cada lenguaje, podemos encontrar varias de distintas fuentes, algo muy común es consultar las guías que ha creado google, por ejemplo para Java podemos encontrar la siguiente\footnote{https://google.github.io/styleguide/javaguide.html}.
\ \\ % asi se hacen 1 saltos de linea

En esta memoria nos centraremos en la guía de estilo para python PEP8. 
Una de las PEP, que hemos explicado antes, que posee Python es PEP8. La guía de estilo PEP8 fue realizada por Guido van Rossum, Barry Warsaw y Nick Coghlan.

Está dedicada a la recopilación de los estándares seguidos por los desarrolladores de python a la hora de escribir código python para la librería estandar. Esta guía está conformada por varias convenciones, en las cuales no entraremos en profundidad, destacaremos una de ellas, alrededor de la cual se basa este proyecto, como ya hemos dicho con anterioridad en esta memoria:``Limitar los tamaños de línea a 79 caracteres como máximo''.
...
\section{Pycodestyle (antes \texttt{pep8})} 
\label{sec:pycodestyle}

Como ya hemos dicho anteriormente cuando hablábamos de PEP8, existen herramientas con las que podemos comprobar de forma automática si nuestro código cumple las reglas que dicta el PEP8.

Aquí vamos a hablar un poco más en profundidad de una de ellas, la cual se llama ``pycodestyle'', este herramienta antes era conocida como ``PEP8'', pero se cambió de nombre para evitar confusiones con la propia guía de estilo.

Este herramienta nos permite, como decimos, pasar un análisis una vez nuestro código está terminado, para ver si cumple calidad. Podemos encontrar información sobre esta herramienta en github\footnote{\url{https://github.com/PyCQA/pycodestyle}}. Pero
básicamente, la forma de utilizarla y lo que nos aporta es lo siguiente:
...
Una vez instalado podemos revisar nuestro código de forma sencilla, por ejemplo, con el siguiente comando, analizamos nuestro fuente: ``optparse.py'':
\begin{verbatim}
...
Como vemos, pycodestyle nos muetra de manera clara la línea y el sitio justo donde estamos cometiendo el error de calidad y por el cual no cumplimos la norma correspondiente de la guía PEP8.
...
Se nos muestra el mensaje del error para el máximo tamaño de línea permitido, el cual tiene que ver con todo este proyecto. Como hemos hablado ya alguna vez, estas herramientas te detectan el error, pero el desarrollador es el que debe tomar tomar le decisión de como
solucionarlo.

\section{Otros programas del estilo de Pycodestyle} 
...
Analizando este dato, vemos que aunque el número de líneas (18.904) nos puede parecer grande en un principio, hay que pensar que tan sólo es el 5,11\%, el cual es un porcentaje bastante bajo.
Al no ser este el grupo de líneas que nos interesa no hemos entrado demasiado en detalle, aunque si que hemos hecho una comprobación en nuestro código. Hemos querido comprobar cuantas de estas líneas pertenecen a comentarios (tanto comentarios \emph{in-line}, como comentarios de varias líneas), para comprobar si este dato está falseado y realmente no
encontramos líneas reales en la zona roja.
...
Esto quiere decir que el grupo conflictivo que nosotros queremos estudiar, ``línea naranjas'', queda en un 5,43\%, lo cual es un porcentaje bajo respecto al total.
