...
En esta línea podemos ver algo que también nos puede pasar. Debido a la forma en la que clasificamos las líneas en los grupos finales, la parte de línea que detecta que ha cambiado es ``status['result']['startup\_status']['code']'' por ``status['startup\_status']['code']''. Podemos observar que al no tener espacios nos lo ha detectado como una palabra y posible variable. Claramente NO estamos ante un caso de los que buscamos.
...
En esta línea vemos que la palabra afectada parece ser una variable. Esta variable ha pasado de ser ``nb\_filter'' a ``filters'', según vemos aquí y sin tener más contexto, como en las demás ocasiones, podemos pensar que esta variable ha podido sufrir el ``truco'' del que hablamos aquí por parte del desarrollador, sin embargo, la clave aquí es estudiar si la variable ha perdido significado con el cambio, en este caso creo que no es así. La razón es que la parte qué se ha eliminado de la variable no me aporta información, es decir, entiendo igual que la variable es un filtro, y el hecho de tener delante ``\_nb'' no me aporta más información. Por lo tanto diría que NO.
...
Nos volvemos a encontrar ante lo que parece una variable, en la que hemos pasado de ``SettingsAttribute'' a ``attr''. Parece claro como decimos, que es una variable, y además en este caso me atrevería a decir que hemos perdido información con el cambio. En el primer nombre tenemos claramente información sobre el significado de esta variable, sin embargo, después del cambio solo tenemos una abreviatura que no nos aportan ninguna información. El código después del cambio ha quedado peor, y parece que el cambio se ha podido dar por el caso que tratamos aquí. Por tanto, añadiría esta línea al SI.
...
La parte afectada aquí no parece ser una variable, hemos pasado de ``logging.info'' a ``log.info''. Claramente no estamos ante nuestro caso ya que esto son unos logs, que han cambiado. Por tanto añadiría otro NO.

\ \\ % asi se hacen 1 saltos de linea

\textbf{Conclusión del análisis manual}: Hemos cogido una pequeña muestra de 10 líneas de nuestros resultados. Hemos obtenido 3 SI y 7 NO. Estos resultados son totalmente subjetivos, y además nos falta mucho contexto sobre el código analizado, ya que estamos tratando sobre líneas totalmente aisladas. Aun así de nuevo hemos obtenido unos resultados negativos respecto al caso buscado.
