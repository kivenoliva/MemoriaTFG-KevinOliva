...
\ \\ % asi se hacen 1 saltos de linea.
Cabe destacar que para realizar nuestro estudio hemos intentado recoger datos del ``mundo real'', es decir, de proyectos reales de software libre, que nos permitan sacar conclusiones reales. Por ello, analizamos repositorios de GitHub, en particular, los repositorios más famosos y de mayor tamaño de Python en dicho portal para formar nuestra base de análisis.
...
\begin{figure}
	\centering
	\includegraphics[scale=0.80]{img/diagramaBarras}
	\caption{Diagrama de barras de los resultados generales}
	\label{fig:resultados_generales}
\end{figure}

Lo primero que nos llama la atención de estos datos es la cantidad de líneas que han salido de cada tipo, es decir, en un principio esperábamos que todo el código analizado, o casi todo, seguiría la guía de estilo PEP8 y, por tanto, apenas encontraríamos ``líneas rojas''.

Analizando este dato, vemos que aunque el número de líneas (18.904) nos puede parecer grande en un principio, hay que pensar que tan sólo supone el 5,11\%, que es un porcentaje bastante bajo.
...
Este análisis nos ha dado como resultado 1.000 líneas, es decir, un 5,28\%. Lo que quiere decir que sí hemos encontrado líneas de código fuente que están en nuestra \emph{zona roja}.
...
Esto quiere decir que el grupo conflictivo que queremos estudiar, las ``línea naranjas'', queda en un 5,43\%.

La gran mayoría de las líneas están por debajo del umbral que marca la guía de estilo PEP8, incluso, por debajo 70 caracteres, es decir, 10 menos del límite. FIXME: no me queda claro esta frase.
Por otro lado, tenemos un pequeño porcentaje de líneas que han pasado de los 80 caracteres y que no están cumpliendo la guía de estilo PEP8. FIXME: ni esta.
...
Una vez que hemos echado un vistazo a los datos totales que hemos analizado, vamos a centrarnos en los distintos repositorios que hemos utilizado para nuestro estudio. Tenemos el siguiente listado: FIXME: esto debería ser una tabla

\begin{enumerate}
	\item \textbf{youtube-dl: }127.737
		
		\begin{itemize}
			\item \textbf{Líneas verdes: }112.898
		\end{itemize}
	
		\begin{itemize}
			\item \textbf{Líneas naranjas: }6.332
		\end{itemize}

		\begin{itemize}
			\item \textbf{Líneas rojas: }8.507, de las cuales 5 son comentarios.(0,058\%)
		\end{itemize}
		
	\item \textbf{SerpentAI: }3.781
		
		\begin{itemize}
			\item \textbf{Líneas verdes: }3.396
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }130
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }282, de las cuales 0 son comentarios.(0\%)
		\end{itemize}
	
	\item \textbf{requests: }33.894
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }29.164
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }1.475
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }3.235, de las cuales 79 son comentarios.(2,44\%)
		\end{itemize}	
	
	\item \textbf{scrapy: }35.283
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }31.174
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }2.400
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }1.709, de las cuales 99 son comentarios.(5,79\%)
		\end{itemize}
	
	\item \textbf{pythondotorg: }11.590
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }10.609
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }380
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }601, de las cuales 3 son comentarios.(0,49\%)
		\end{itemize}
	
	\item \textbf{models: }60.253
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }56.246
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }2.949
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }1.058, de las cuales 683 son comentarios.(0,64\%)
		\end{itemize}
	
	\item \textbf{lbry: }28.417
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }24.832
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }1.700
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }1.885, de las cuales 59 son comentarios.(3,12\%)
		\end{itemize}
		
	\item \textbf{keras: }54.409
		
		\begin{itemize}
			\item \textbf{Líneas verdes: }49.456
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }3.428
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }1.525, de las cuales 60 son comentarios.(3,93\%)
		\end{itemize}
	
	\item \textbf{flask: }14.436
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }13.030
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }1.307
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }99, de las cuales 11 son comentarios.(11,11\%)
		\end{itemize}
	
	\item \textbf{awesome-python: }76
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }70
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas naranjas: }3
		\end{itemize}
		
		\begin{itemize}
			\item \textbf{Líneas rojas: }3, de las cuales 1 son comentarios.(33,3\%)
		\end{itemize}
	
	\item \textbf{awesome-machine-learning': }38
	
		\begin{itemize}
			\item \textbf{Líneas verdes: }35
		\end{itemize}
		...
		\begin{itemize}
			\item \textbf{Líneas rojas: }0
		\end{itemize}
					
\end{enumerate}

Como podemos ver, hemos utilizado repositorios distintos, tanto en tamaño como en características. Lo más destacable es:
...
Como podemos ver son uno porcentajes bajísimos. Si analizamos esas 396 líneas, y las clasificamos según su motivo, de la forma que ya explicamos en el capítulo 4 (véase la sección~\ref{sec:clasificacion-de-lineas-conflictivas}), obtenemos el siguiente resultado:s
...
\begin{itemize}
	...
	\begin{verbatim}
	Línea Actual ---------->         'url':
	 'http://91porn.com/view_video.php?viewkey=7e42283b4f5ab36da134',
	\end{verbatim}
	\begin{verbatim}
	Línea en el Pasado ---->             'url':
	 'http://91porn.com/view_video.php?viewkey=7e42283b4f5ab36da134',
	\end{verbatim}
	...
	Ejemplo:
	\begin{verbatim}
	Línea Actual ---------->             'subtitles':
	 self.extract_subtitles(video_id, subtitles_id, hl),
	\end{verbatim}
	\begin{verbatim}
	Línea en el Pasado ---->             'subtitles':
	 self.extract_subtitles(video_id, video_subtitles_id, hl),
	\end{verbatim}
	...
	\begin{verbatim}
	Línea Actual ---------->                 'https:
	//tv.nrk.no/program/Episodes/{series}/{season}'.format(
	\end{verbatim}
	\begin{verbatim}
	Línea en el Pasado ---->             self.url_result('https:
	//tv.nrk.no/program/Episodes/{series}/{season}'.format(
	\end{verbatim}
	...
	\begin{verbatim}
	Línea Actual ---------->         'url':
	 'http://www.chilloutzone.net/video/eine-sekunde-bevor.html',
	\end{verbatim}
	\begin{verbatim}
	Línea en el Pasado ---->     	u'url':
	 u'http://www.chilloutzone.net/video/enemene-meck-alle-katzen-weg.html',
	\end{verbatim}
...
Como ya vimos, tenemos dos grupos que pueden entrar dentro de nuestro estudio, que son: ``La línea se acorta exclusivamente por que una palabra pasa a ser más corta'' y ``La línea se acorta por tabulaciones y aparte porque una sola palabra pasa a ser más corta''. Si los agrupamos y hablamos de porcentajes en función de las 396 líneas que ya hemos filtrado
...
Si comparamos este último, filtrado con los datos iniciales, tenemos que \textbf{128 líneas de 369.911} pueden haber sufrido el cambio que explicamos en este proyecto a propósito. Lo cual es un \textbf{0.034\%}. Este dato es un porcentaje ínfimo, que da a entender que el debate sobre esta cuestión carece de sentido, o al menos, no debería tener la importancia que se le ha dado.
...
En este caso vemos que la palabra que se acorta no parece mucho una variable que haya creado el desarrollador. No tenemos más contexto del resto del código que tiene esta línea (las líneas colindantes), pero por el nombre de la variable inicial: ``\_VALID\_URL'' y por el contenido de esta variable, parece que la parte que se acorta no es una variable en sí (display\_id a id). Se puede pensar que depende del programa, el desarrollador esté creando la URL y decida su contenido, pensando así que lo ha podido acortar por el motivo que perseguimos en nuestro análisis. Sin embargo, en esta situación no parece que se dé el caso, ya que normalmente se trabaja con URLs que ya existen, y no parece ser que el desarrollador haya acortado una variable por el motivo que tratamos. Por tanto, creemos que en este caso no se trata de un cambio en el nombre de la variable con el fin de cumplir con la guía de estilo PEP8.
\ \\ % asi se hacen 1 saltos de linea
...
En esta línea, así a bote pronton, parece que SÍ se trata de un cambio para cumplir con la guía de estilo PEP8. Se puede ver de manera fácil que lo que ha cambiado es una variable, ya que es un parámetro que se le está pasando a una función. La variable ha pasado de ser ``playlist-title'' a simplemente ``title''. Podemos afirmar que el nombre de la variable ha perdido sentido, ya que llamar a una variable \emph{título} no es muy buena elección. Antes se sabía que era el título de una playlist, sin embargo ahora es más difícil adivinar el contenido de esa variable. A nuestro entender, el código ha quedado peor que el original.
...
En esta línea tenemos un caso en el que parece claro que la palabra que se ha acortado es una variable. Sin embargo, no parece que la variable haya perdido sentido respecto a su nombre anterior por este motivo, ya que lo único que se ha hecho ha sido quitar un guión bajo inicial. Por tanto para esta línea también la clasificaríamos como que NO.
...
\textbf{Par de líneas}:
\begin{verbatim}
Línea Actual ---------->         except (TimeoutError,
 HTTPException, SocketError, ProtocolError) as e:
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->         except (TimeoutError,
 HTTPException, SocketError, ConnectionError) as e:
\end{verbatim}

\textbf{Análisis}:

En este caso, vemos que la línea cumple el patrón de nuestro análisis para estar aquí. Sin embargo, si la analizamos vemos que efectivamente la palabra afectada es una variable, que ha pasado de ``ConnectionError'' a ``ProtocolError''. Si nos fijamos en el nombre de la variable no parece que este cambio haya sido para acortar la misma. Más bien parece que ahora la variable se refiere a otra cosa y ha cambiado de nombre, el cual no ha perdido significado. No parece que el motivo sea mantener la misma variable pero acortando el nombre, por tanto, diríamos que NO.
...
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->             elif 
status['result']['startup_status']['code'] == LOADING_WALLET_CODE:
\end{verbatim}

\textbf{Análisis}:

En esta línea podemos ver algo que también nos puede pasar. Debido a la forma en la que clasificamos las líneas en los grupos finales, la parte de línea que detecta que ha cambiado es ``status['result']['startup\_status']['code']'' por ``status['startup\_status']['code']''. Podemos observar que al no tener espacios, nuestra herramienta lo ha detectado como una palabra y posible variable. Claramente NO estamos ante un caso de los que buscamos.

\ \\ % asi se hacen 1 saltos de linea

\textbf{Par de líneas}:
\begin{verbatim}
Línea Actual ---------->         `(batch, new_rows, new_cols,
 filters)` if data_format='channels_last'.
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->         `(batch, new_rows, new_cols,
 nb_filter)` if data_format='channels_last'.
\end{verbatim}

\textbf{Análisis}:

En esta línea vemos que la palabra afectada parece ser una variable. Esta variable ha pasado de ser ``nb\_filter'' a ``filters'', según vemos aquí y sin tener más contexto. Como en ocasiones anterioes, podemos pensar que esta variable ha podido sufrir el ``truco'' del que hablamos aquí por parte del desarrollador.  Sin embargo, la clave aquí está en estudiar si la variable ha perdido significado con el cambio. En este caso creo que no es así. La razón es que la parte que se ha eliminado de la variable no aporta información, es decir, se entiende igual que la variable es un filtro y el hecho de tener delante ``\_nb'' no me aporta mayor información. Por lo tanto diríamos que NO.

\ \\ % asi se hacen 1 saltos de linea

\textbf{Par de líneas}:
\begin{verbatim}
Línea Actual ---------->     def _download_name(self, name,
 timeout=None, download_directory=None,
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->     def _download_name(self, name,
 timeout=conf.settings.download_timeout, download_directory=None,
\end{verbatim}

\textbf{Análisis}:

En esta línea vuelve a parecer que efectivamente la palabra afectada parece una variable. Sin embargo, nos encontramos ante un caso distinto al que buscamos. El motivo es que no sólo se ve afectada una variable, sino una variable y su asignación. La expresión afectada pasa de ser ``timeout=conf.settings.download\_timeout'' a ser ``timeout=None''. Este cambio parece ser un cambio que se ha tenido que realizar en el código, ya que esa variable ahora pasa a valer otra cosa. Parece claro que no se trata de un caso que buscamos, por tanto, apuntamos otro NO.

\ \\ % asi se hacen 1 saltos de linea

\textbf{Par de líneas}:
\begin{verbatim}
Línea Actual ---------->                 'lynda returned
 error: %s' % video['Message'], expected=True)
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->                 'lynda returned
 error: %s' % video_json['Message'], expected=True)
\end{verbatim}

\textbf{Análisis}:

Nos encontramos ante un caso en el que la parte de la línea afectada no está claro si es una variable, en mi opinión, lo que se ha acortado aquí puede ser el nombre de una función o método. La parte afectada ha pasado de ser ``video\_json['Message']'' a ser ``video['Message']''. En este caso, puede que no demasiado, pero me atrevería a decir que el nombre afectado ha perdido información, ya que el nombre antiguo: ``video\_json'' no es demasiado explicativo. Sin embargo, anular la parte ``\_json'' resta una información que puede ser importante, ya que habla sobre un formato. Por tanto, en esta ocasión me atrevería a decir que SÍ ha ocurrido el caso que buscamos.

\ \\ % asi se hacen 1 saltos de linea

\textbf{Par de líneas}:
\begin{verbatim}
Línea Actual ---------->         with mock.patch.object
(attr, '__setattr__') as mock_setattr, \
\end{verbatim}
\begin{verbatim}
Línea en el Pasado ---->         with mock.patch.object
(SettingsAttribute, '__setattr__') as mock_setattr, \
\end{verbatim}

\textbf{Análisis}:

Nos volvemos a encontrar ante lo que parece una variable, en la que hemos pasado de ``SettingsAttribute'' a ``attr''. Parece claro, como decimos, que es una variable, y además en este caso podemos decir que hemos perdido información con el cambio. En el primer nombre tenemos claramente información sobre el significado de esta variable. Sin embargo, después del cambio solo tenemos una abreviatura que no nos aportan ninguna información. El código después del cambio ha quedado peor, y parece que el cambio se ha podido dar por el caso que tratamos aquí. Por tanto, añadiríamos esta línea al SI.
