...
Los lenguajes de programación suelen tener una guía de estilo que indica, entre otras cuestiones, cómo ha de formatearse el código. Este proyecto pretende realizar un estudio sobre el uso de una de las reglas que contiene la guía de estilo de Python PEP8\footnote{\url{http://https://pythonwiki.wikispaces.com/pep8}}.
...
Si estamos leyendo código y pasamos por esta línea, se entiende bastante bien, sin necesidad de contar con más contexto. Se puede suponer fácilmente que estamos obteniendo los subtítulos, que obtenemos de una función llamada ``extract-subtitles''. El nombre de la función parece que se ha elegido con sentido, al ser informativo. También sabemos que para ello le pasamos como parámetros el id del vídeo y el id de los subtítulos.
...
\begin{figure}[h!]
	...
	\caption{Diagrama de flujo del funcionamiento del estudio. FIXME: Evita el "nos recorremos"; modifícalo a "recorremos". De cada repositorio,, tiene dos comas. Si es un fichero con extensión .py. Creamos un objeto en la BBDD. A veces hablas en primera personal plural, otras veces en reflexivo.}
	\label{fig:diagrama}
\end{figure}
...
\begin{figure}
	...
	\caption{Salida de la instrucción \texttt{git log} con opciones. FIXME: especificar qué opciones.}
	\label{fig:log2}
\end{figure}
...
\begin{enumerate}
	...
	\item La línea se diferencia porque se han eliminado 4 espacios (tabulaciones), y además ha cambiado una sola palabra, que se ha acortado (posible caso buscado).
...
\begin{itemize}
	\item \textbf{La línea se diferencia por el único cambio de una palabra que ha pasado a ser más corta: }Partimos tanto la línea antigua como la Línea actual por espacios, de manera que nos quedamos con dos arrays que guardan las palabras de cada línea. Comparamos estos dos arrays, y si sólo se diferencian en una posición, querrá decir que solo hay una palabra distinta. Esta palabra puede que sea una variable, por tanto se añade a este grupo.
\end{itemize}
...
\begin{figure}
	...
	\caption{Diagrama de barras de los resultados generales. FIXME: en el diagrama pone analzadas!}
...
Como podemos ver son uno porcentajes bajísimos. Si analizamos esas 396 líneas, y las clasificamos según su motivo, de la forma que ya explicamos en el capítulo 4 (véase la sección~\ref{sec:clasificacion-de-lineas-conflictivas}), obtenemos el siguiente resultados\footnote{En los ejemplos que se muestran \emph{verbatim} en esta memoria, se ha omitido la indentación de las líneas para que quepan en el ancho de página.}:
...
Dentro de Express vamos a utilizar la librería de Express Generator, que nos crea una estructura base para una aplicación.\\
